  ...
  server.route([
    {  //get one
      ...
      handler: function(request, reply) {
        reply ("Hello, i am an awesome HarryQuote API server!!");
      }
    },
    { // get all
      method:'GET',
    ...
    { // get by ID
      ...
      handler: function (request,reply) {
        ...
        if (Number.isNaN(id)) {
          return reply("comeon, give me a number,bro").code(404);
        }
        reply( dB[id] );
      }
    },
    ...
    { // PUT update one quote
      method: 'PUT',
      path: '/quotes/{id}',
      handler: function (request, reply) {
        var id = Number(request.params.id);
        if (Number.isNaN(id)) {
          return reply("dude, you want to amend, give me the number").code(404);
        }
        var amendQuote =request.payload.quote.quote;
        var amendAuthor =request.payload.quote.author;
        dB[id].quote.quote = amendQuote;
        dB[id].quote.author = amendAuthor;
          reply(db[id]);
